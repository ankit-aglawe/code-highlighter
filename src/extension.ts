...
// Map from file URI to its highlights
let fileHighlights: Map<string, HighlightDecoration[]> = new Map();
...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.window.onDidChangeActiveTextEditor(editor => {
        if (editor) {
            ...
            const highlights = fileHighlights.get(fileUri) || [];
            highlights.forEach(({ range, decoration }) => {
                editor.setDecorations(decoration, [range]);
            });
        }
    });
    ...
    context.subscriptions.push(
        vscode.languages.registerHoverProvider({ scheme: 'file', language: '*' }, {
            provideHover(document, position, token) {
                ...
                const highlights = fileHighlights.get(fileUri) || [];
                const highlight = highlights.find(({ range }) => range.contains(position));

                let hoverContent: vscode.MarkdownString;

                if (highlight) {
                    // Prepare hover content based on whether the highlight is saved
                    if (highlight.isSaved) {
                        ...
                        hoverContent = new vscode.MarkdownString(
                            `💾 [Save Highlight](command:extension.saveHighlight) | ✏️ [Erase Highlight](command:extension.removeHighlight) | 🎨 [Change Highlight Color](command:extension.changeHighlightColor)`
                        );
                    }
                } else {
                    hoverContent = new vscode.MarkdownString(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('extension.highlightCode', async () => {
            ...
            const { highlighted, range } = getHighlightedText(editor);
            if (!range || isRangeHighlighted(range, editor.document.uri.toString())) {
                return; // Prevent duplicate highlights
            }
            ...
            const fileUri = editor.document.uri.toString();
            const highlight: HighlightDecoration = {
                range,
            ...
            if (!fileHighlights.has(fileUri)) {
                fileHighlights.set(fileUri, []);
            }
            fileHighlights.get(fileUri)?.push(highlight); // Track this decoration

            vscode.window.showInformationMessage(`Code highlighted with ${colorName}!`);
        })
    );

    // Command to save highlight
    context.subscriptions.push(
        vscode.commands.registerCommand('extension.saveHighlight', () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                return;
            }
            ...
            if (highlight && !highlight.isSaved) {
                highlight.isSaved = true;
                vscode.window.showInformationMessage('Highlight saved!');
                // Save highlights
                saveHighlightsToFile();
            }
        })
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('extension.removeHighlight', () => {
            ...
            if (index !== -1) {
                const { decoration } = highlights[index];
                editor.setDecorations(decoration, []); // Remove highlight
                decoration.dispose(); // Dispose of the decoration type
                highlights.splice(index, 1); // Remove from tracking
                vscode.window.showInformationMessage('Highlight removed!');
                // Save highlights
                saveHighlightsToFile();
            }
        ...
        vscode.commands.registerCommand('extension.changeHighlightColor', async () => {
            ...
            if (highlight) {
                ...
                if (selectedColor) {
                    ...
                    // Apply the new decoration
                    editor.setDecorations(newDecoration, [highlight.range]);

                    // Update the decoration in the array
                    highlight.decoration = newDecoration;
                    highlight.color = selectedColor.color;

                    vscode.window.showInformationMessage(`Highlight color changed to ${selectedColor.label}!`);

                    // Save highlights if the highlight is saved
                    if (highlight.isSaved) {
                        saveHighlightsToFile();
                    }
                }
    ...
    function saveHighlightsToFile() {
        ...
        const highlightsToSave: any[] = [];

        for (const [fileUri, highlights] of fileHighlights) {
            const savedHighlights = highlights
                .filter(highlight => highlight.isSaved)
                .map(({ range, color }) => ({
                    fileUri,
                    start: { line: range.start.line, character: range.start.character },
                    end: { line: range.end.line, character: range.end.character },
            ...
            highlightsToSave.push(...savedHighlights);
        }
    ...
    // Function to load highlights from file
    function loadHighlights() {
        ...
        savedHighlights.forEach((savedHighlight: any) => {
            const fileUri = savedHighlight.fileUri;
            const range = new vscode.Range(
                new vscode.Position(savedHighlight.start.line, savedHighlight.start.character),
                new vscode.Position(savedHighlight.end.line, savedHighlight.end.character)
            );
            ...
            fileHighlights.get(fileUri)?.push(highlight);

            // Apply the decoration if the document is open
    ...
    // Helper function to check if a range is already highlighted
    function isRangeHighlighted(range: vscode.Range, fileUri: string): boolean {
        const highlights = fileHighlights.get(fileUri) || [];
        return highlights.some(({ range: highlightedRange }) => highlightedRange.isEqual(range));
    }
}
...
export function deactivate() {
    ...
    for (const highlights of fileHighlights.values()) {
        highlights.forEach(({ decoration }) => decoration.dispose());
    }
